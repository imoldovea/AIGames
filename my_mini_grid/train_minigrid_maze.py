...
def train_model(env, log_dir):
    ...
    # Create a specific tensorboard log directory
    tensorboard_log_dir = os.path.join(log_dir, "tensorboard")
    os.makedirs(tensorboard_log_dir, exist_ok=True)

    # Configure gym logger with proper tensorboard logging
    ...
    # Make sure tensorboard_log points to the correct directory
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
        tensorboard_log=tensorboard_log_dir,  # Use specific tensorboard directory
        learning_rate=3e-4,  # Add some explicit hyperparameters for logging
        n_steps=2048,
        batch_size=64,
        n_epochs=10,
        gamma=0.99,
    ...
    # Set the logger on the model
    model.set_logger(gym_logger)

    # Train the model with explicit callback for logging
    model.learn(
        total_timesteps=Config.TRAINING_TIMESTEPS,
        progress_bar=True,
        tb_log_name="PPO_MazePool",  # Give a specific name for tensorboard logs
...
def test_model(model, base_test_env, enable_visual=False):
...
def main():
    ...
    # Test the model - set enable_visual=True to see visual display, False to disable
    test_model(model, base_test_env, enable_visual=False)

...
def load_loss_data():
    try:
        ...
        df['exit_accuracy'] = pd.to_numeric(df['exit_accuracy'], errors='coerce')  # newly added column
...
app.layout = html.Div([
    ...
    dcc.Graph(id="exit-accuracy-graph"),  # new plot for exit accuracy
...
@app.callback(
    ...
    Output("exit-accuracy-graph", "figure"),  # new plot output
...
def update_graphs(n):
    ...
    if df.empty or 'model' not in df.columns:
        return no_update, no_update, no_update, no_update, no_update, no_update  # added one no_update return
    ...
    for fig in (fig_training, fig_validation, fig_accuracy, fig_val_accuracy, fig_exit_accuracy, fig_time_per_step):
        fig.update_layout(
            xaxis_title="Epoch",
            legend=dict(
                x=1.02,
    ...
    return fig_training, fig_validation, fig_accuracy, fig_val_accuracy, fig_exit_accuracy, fig_time_per_step
